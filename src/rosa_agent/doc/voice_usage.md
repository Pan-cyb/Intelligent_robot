...
ASR_COMMAND_LISTEN_TIMEOUT_SEC=15
ASR_COMMAND_WINDOW_SEC=25
ASR_SESSION_IDLE_TIMEOUT_SEC=60
ASR_POST_TTS_COOLDOWN_SEC=0.8
```

`ASR_VAD_LISTEN_TIMEOUT_SEC=0` 表示等待唤醒词时一直监听。`ASR_COMMAND_LISTEN_TIMEOUT_SEC` 控制每次命令监听的最长时间，`ASR_COMMAND_WINDOW_SEC` 控制唤醒后首次等待命令的总窗口，`ASR_SESSION_IDLE_TIMEOUT_SEC` 控制连续对话中每轮回答后的空闲超时；超时后重新等待唤醒词。
